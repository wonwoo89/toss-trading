...
function applyChartViewportWhenReady(
  chart: IChartApi,
  viewport: ChartViewport,
  attempt = 0,
) {
...
export function CandleChart({
  ...
  useEffect(() => {
    ...
    const visibleRange =
      prependedCount > 0 ? chart.timeScale().getVisibleLogicalRange() : null
    ...
    if (visibleRange && prependedCount > 0) {
      ...
      const wasNearRealtime = isNearRealtimeViewport(
        liveViewport,
        chart,
  ...
  const isUpCandle =
    hoveredCandle !== null && hoveredCandle.close >= hoveredCandle.open
