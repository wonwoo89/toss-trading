...
export function buildBuyBreakEvenHint(
  buyPrice: number,
  commissionRatePercent: number,
): string {
  const breakEven = calculateRoundTripBreakEvenSellPrice(buyPrice, commissionRatePercent)
  const markup =
    buyPrice > 0 ? ((breakEven - buyPrice) / buyPrice) * 100 : undefined
  const markupText =
    markup !== undefined ? formatSignedPercent(markup / 100, markup) : undefined
  return markupText
