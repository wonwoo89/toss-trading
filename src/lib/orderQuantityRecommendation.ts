...
function getBasePercent(side: 'BUY' | 'SELL', level: ReturnType<typeof buildChartSignalSnapshot>['level']) {
  if (side === 'BUY') {
    switch (level) {
      case 'strong_buy':
        return 50
...
function getSignalReason(side: 'BUY' | 'SELL', level: ReturnType<typeof buildChartSignalSnapshot>['level']) {
  if (side === 'BUY') {
    switch (level) {
      case 'strong_buy':
        return '강매수 신호'
...
export function buildOrderQuantityRecommendation(
  ...
  const atrRatio =
    atr !== undefined && unitPrice > 0 ? atr / unitPrice : undefined
  ...
  const quantity = recommended
    ? Math.max(1, Math.floor(maxQuantity * (percent / 100)))
    : undefined
  const amountUsd =
...
export function formatQuantityRecommendationValue(
  recommendation: OrderQuantityRecommendation,
) {
