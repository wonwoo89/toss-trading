...
function getMacdMetric(closes: number[]): ChartSignalMetric {
  ...
  const bias: ChartSignalBias = histogram > 0 ? 'bullish' : histogram < 0 ? 'bearish' : 'neutral';
...
function getMomentumMetric(candles: ChartCandle[]): ChartSignalMetric {
  ...
  const gapPercent = previous.close > 0 ? ((last.open - previous.close) / previous.close) * 100 : 0;
...
function getVolumeProfileMetric(candles: ChartCandle[]): ChartSignalMetric {
  ...
  const delta = lastClose > 0 ? ((lastClose - peakPrice) / lastClose) * 100 : undefined;
...
export function buildExtendedIndicatorMetrics(candles: ChartCandle[]): ChartSignalMetric[] {
  const sorted = candles.slice().sort((a, b) => a.time - b.time);
