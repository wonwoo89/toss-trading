...
sudo apt install -y nodejs git rsync   # rsync: CI 자동 배포가 dist 를 전송할 때 사용
...
> **평소엔 main 에 머지하면 CI 가 자동 배포**한다(아래 "자동 배포" 섹션). 서버에서 직접 빌드할
> 일이 없어 CPU 스파이크 걱정이 없으므로 그 경로를 권장한다.
> 아래 수동 방법은 CI 없이 서버에서 직접 돌릴 때만 쓴다. 이때 `build:all` 이 CPU 를 많이 써
> **버스트 인스턴스에선 CPU 크레딧 소진으로 일시적으로 느려질 수 있다**(빌드만 잠깐).

**스크립트 한 방** — `update.sh` 가 pull→install→build→restart→health 까지 자동:
...
**러너에서 빌드**한 뒤, 테일넷에 임시로 합류해 결과물(`dist`)을 서버로 보내고 재시작시킨다.
**공개 포트는 하나도 열지 않고**(러너는 GitHub 로 outbound 만 사용), **SSH 개인키도 두지 않는다**
(인증은 Tailscale SSH = 테일넷 신원으로 처리).

> **빌드는 러너에서** 한다. WASM(asc) + vite 빌드는 CPU 를 많이 써서 작은 버스트
> 인스턴스에서 돌리면 CPU 크레딧을 소진시켜 throttle/먹통을 유발하기 때문이다.
> 서버는 빌드 없이 `dist` 를 받아 재시작만 하므로 CPU 스파이크가 없다.

워크플로 파일: `.github/workflows/deploy.yml` (이미 레포에 있음).

```
[main 머지] → CI(ci.yml) 통과 → Deploy(deploy.yml)
   러너: pnpm install + build:all → dist 생성 (서버 CPU 0)
   러너: tag:ci 로 테일넷 합류
       → rsync dist → 서버(tag:prod)
       → ssh: SKIP_BUILD=1 ./update.sh  (git pull + 의존성 + pm2 restart, 빌드 없음)
```
